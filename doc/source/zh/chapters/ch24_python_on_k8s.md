...
- 生产环境使用 `uvicorn` 时设置 `--workers N`（N = 2 × CPU 核心数 + 1）
- 通过 `WEB_CONCURRENCY` 环境变量动态控制 Worker 数量
- 使用 `gunicorn -k uvicorn.workers.UvicornWorker` 来管理进程
- 务必设置内存限制 —— Python 应用的内存消耗可能较大
- Celery 建议配置 `--max-tasks-per-child` 以防止内存泄漏
